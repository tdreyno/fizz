...
Use this table when mapping `.listen("event", handler)` calls to convenience helper methods on browser DOM builders.

Example:

- `dom.document().listen("mousedown", toAction)` -> `dom.document().onMouseDown(toAction)`

All helper names below are generated from the same event maps that type each target builder.
